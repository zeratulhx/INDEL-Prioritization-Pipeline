...
```{r}
#Extract contig ID for VAF matching from matching_Id_df

matching_contigs_df<- subset(matching_rows_df, select = c(file_name, V3, col4))
matching_contigs_list <- matching_contigs_df[,2]
write(matching_contigs_list, file = "/home/briana/shared/contigslistforCOSVgenes.txt")
...
VAF_COSMIC_values <- VAF_COSMIC_values %>% rename("VAF" = V6)
```
...
```
```{r}
#Generate VAF plots for top 20 genes by count MODERATE and HIGH impact, no associated database entry 
...
top_vals <- names(sort(table(new_df$col4), decreasing = TRUE)[1:top_n])
